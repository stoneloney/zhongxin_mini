<template>
	<view>
	<view class="address" v-if="!hasAddress">
	  <view class="address_a">
		  ...
		  <view class="img2"></view>
			<view hover-class="none" @click="link('/pages/user/edit_address')" class="buy"  >+ 新增地址</view>
	  </view>
	</view>
	
	<view class="hed" v-if="hasAddress">
	    <view class="hd" v-for="(item, index) in addresses">
	    <view  class="hed_a">
	    <view class="content" bindtouchstart="touchS" bindtouchmove="touchM">
			<view class="fl" bindtap="xiugai">
				<view class="tex1">
				  ...
				  <text class="moren" v-if="item.is_default === 1">默认</text>
				</view>
				<view class="tex2"> {{ item.address }}</view>
			</view>
			<view class="fr"  hover-class="none" @click="delAddress(item.id)">
				<image src="../../static/shanchu.png"></image>
			</view>
			<view class="fr"  hover-class="none" @click="editAddress(item.id)">
...
<script>
	...
	import { addressList, addressOperate } from '@/api/address'
